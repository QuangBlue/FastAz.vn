...
# Python imports
import sys
# sys.path.append("/".join(x for x in __file__.split("/")[:-1]))      ????? chua biet cai nay lam gi
...
class FirebaseLoginScreen(Screen, EventDispatcher):
    ...
    function properly.
    2) You probably want to wrap the firebaseloginscreen in a ScreenManager. => Done
    ...
    debug = False  # Setting up debug state, yes => showing debugging message in terminal, no => not showing
